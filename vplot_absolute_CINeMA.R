...
palette <- c(hcl.colors(5, palette = 'Purples', rev = T, alpha = 0.7)[2:5], '#DDF1FB')

# CINeMA
nrp$q <- 'CINeMA'
nrp$q[nrp$drug=='treatment 1'] <- c('moderate', 'low', 'very low', 'moderate', 'low')
nrp$q[nrp$drug=='treatment 2'] <- c('low', 'high', 'high', 'high', 'high')
nrp$q[nrp$drug=='placebo'] <- c('NA', 'NA', 'NA', 'NA', 'NA')
nrp$q <- factor(nrp$q, levels = c('very low', 'low', 'moderate', 'high', 'NA'))
...
linespace <- 5 # [ACTION] distance between lines, change as needed
...
title1.coord <- 5.5 # [ACTION] position of section title1 text
...
# folder prep
setwd('~/Desktop/V plot/graph8.0 - for V plot/figure3') # [ACTION] change wd accordingly
...
nightingale <- function (drugname) {
  ggplot(drugname, aes(x = outcome, y = value)) +
    ...
    geom_col(fill = palette[drugname$q], width = 1) +
    ...
    geom_vline(xintercept = seq(0.5, piecount+0.5, by = 1), color = '#f9fcff', size = 1.2, alpha = 0.7) +
    geom_hline(yintercept = seq(linespace*2, eracle-linespace, by = linespace*2), color = '#002147', size = 0.3, alpha = 0.2) +
    geom_hline(yintercept = seq(linespace, eracle-linespace, by = linespace*2), color = '#002147', size = 0.55, alpha = 0.2, linetype = 'dotted') +
...
plot.new()
legend('center', title='CINeMA ratings', legend = c('very low', 'low', 'moderate', 'high'), fill = palette[1:4], horiz = F, cex = 1.5)
dev.off()
...
pangea <- lapply(unique(nrp$drug), multiple.plots) # [ACTION] change wd accordingly
...
some.plots <- function (x) {
  ...
  all.plots.img <- image_append(all.plots, stack = TRUE)
  image_write(all.plots.img, path = paste0('figure3.jpeg'), format = 'jpeg', density = 1200, quality = 100)
}
